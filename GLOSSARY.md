...
## Debate Skill
A `/debate` skill at `orchestrator/skills/` that runs a multi-agent adversarial session. Parallel logic and feelings debaters argue from their designated files; an observer synthesizes; the orchestrator concludes. Variant of grill-with-docs with multi-agent execution.
_Avoid_: debate mode, discussion skill

## Logic Debater
Claude agent reading CLAUDE.md. Argues from code structure and implementation facts during a debate session.
_Avoid_: code debater, Claude agent

## Feelings Debater
Gemini agent reading GEMINI.md. Argues from narrative, documentation, and human context during a debate session.
_Avoid_: narrative debater, Gemini agent

## Maintainer
Codex agent reading and writing AGENTS.md. Implements work items from Issues; accumulates implementation notes into AGENTS.md after each sprint.
_Avoid_: implementor, coder, developer agent

## Observer
Qwen agent reading QWEN.md. Holds the comprehensive view of the system (code + docs + domain + implementation history in Traditional Chinese) during a debate session.
_Avoid_: watcher, monitor agent

## Orchestrator (Debate)
Main Claude thread that leads a debate session. Synthesizes output from logic debater, feelings debater, and observer into a final conclusion.
_Avoid_: coordinator, moderator

## AGENTS.md
Code understanding file for each directory. Generated by holistic-ai on first run; Codex-owned thereafter. Codex reads and writes implementation notes here. holistic-ai reads AGENTS.md on re-run but does not overwrite it.
_Avoid_: AI docs, agent instructions file

## CLAUDE.md
Independent copy of AGENTS.md written by holistic-ai at generation time. Re-generated by holistic-ai on each re-run (reads AGENTS.md as source, writes CLAUDE.md fresh). Read by Claude (logic debater). Not a symlink.
_Avoid_: Claude symlink, AGENTS.md copy

## GEMINI.md
Documentation-grade prose file generated by Gemini. Source: AGENTS.md + all domain docs. Audience: new developer or external reader. Read by Gemini (feelings debater). Not a symlink.
_Avoid_: Gemini symlink, documentation file

## QWEN.md
Comprehensive Traditional Chinese (繁體中文) synthesis. Source: GEMINI.md + domain docs + AGENTS.md (Codex implementation notes). Read by Qwen (observer). Most complete per-directory file.
_Avoid_: Chinese translation, Qwen symlink
